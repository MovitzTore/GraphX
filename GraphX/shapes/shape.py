...
class Shape:
    ...
    # ========== COLLISION METHODS (is_touching and friends) ==========
    
    def is_touching(self, other) -> bool:
        """Check if this shape is touching another shape or the window borders.
        ...
        """
        from ..framework import get_engine
        
        engine = get_engine()
        
        # check borders
        if isinstance(other, str) and other.lower() == "borders":
            if engine is None or engine.screen is None:
                return False
            
            # get shape bounds
            if self.shape_type == "rectangle":
                left = self.x
                right = self.x + (self.width * self._scale_x)
                top = self.y
                bottom = self.y + (self.height * self._scale_y)
                ...
                left = self.x - radius_x
                right = self.x + radius_x
                top = self.y - radius_y
                bottom = self.y + radius_y
            else:  # pixel
                left = right = self.x
                top = bottom = self.y
            
            # check collision with window edges
            if left <= 0 or right >= engine.width or top <= 0 or bottom >= engine.height:
                return True
            return False
        
        # check collision with another shape
        elif isinstance(other, Shape):
            return engine.check_collision(self, other) if engine else False
        
        else:
            error_msg = f"is_touching() expects a Shape or 'borders', got {type(other).__name__}"
            if engine and engine.error_handler:
                engine.error_handler.show_fatal_error(engine.screen, error_msg)
            raise TypeError(error_msg)
    
    def touching_borders(self) -> bool:
        """Check if shape is touching the window borders."""
        return self.is_touching("borders")
    
    def touching_any(self, shapes: List['Shape']) -> Optional['Shape']:
        """Check if shape is touching any shape in a list. Returns the first shape touched or None."""
        for other in shapes:
            if self.is_touching(other):
                return other
        return None
    
    def touching_all(self, shapes: List['Shape']) -> List['Shape']:
        """Returns list of all shapes this shape is touching."""
        touching = []
        for other in shapes:
            if self.is_touching(other):
    ...
    @property
    def touching_border(self) -> bool:
        """True if shape touches any window border."""
        return self.is_touching("borders")
    
    @property
    def at_left_edge(self) -> bool:
        """True if shape touches left edge."""
        engine = get_engine()
        if engine is None:
            return False
        ...
        if self.shape_type == "rectangle":
            ...
            radius_x = (self.radius_x if hasattr(self, 'radius_x') else self.radius) * self._scale_x
            return self.x - radius_x <= 0
        return self.x <= 0
    
    @property
    def at_right_edge(self) -> bool:
        """True if shape touches right edge."""
        engine = get_engine()
        if engine is None:
            return False
        
        if self.shape_type == "rectangle":
            return self.x + (self.width * self._scale_x) >= engine.width
        elif self.shape_type in ["circle", "oval"]:
            radius_x = (self.radius_x if hasattr(self, 'radius_x') else self.radius) * self._scale_x
            return self.x + radius_x >= engine.width
        return self.x >= engine.width
    
    @property
    def at_top_edge(self) -> bool:
        """True if shape touches top edge."""
        engine = get_engine()
        if engine is None:
            return False
        ...
        if self.shape_type == "rectangle":
            ...
            radius_y = (self.radius_y if hasattr(self, 'radius_y') else self.radius) * self._scale_y
            return self.y - radius_y <= 0
        return self.y <= 0
    
    @property
    def at_bottom_edge(self) -> bool:
        """True if shape touches bottom edge."""
        engine = get_engine()
        if engine is None:
            return False
        
        if self.shape_type == "rectangle":
            return self.y + (self.height * self._scale_y) >= engine.height
        elif self.shape_type in ["circle", "oval"]:
            radius_y = (self.radius_y if hasattr(self, 'radius_y') else self.radius) * self._scale_y
            return self.y + radius_y >= engine.height
        return self.y >= engine.height
